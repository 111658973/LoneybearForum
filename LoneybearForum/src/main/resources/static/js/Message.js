    ...
    methods:{
        ...
        openSocket() {
            if (typeof WebSocket == "undefined") {
                ...
                var socketUrl ="http://localhost:8081/message/websocket/connect/"+ this.userId;
        ...
        getUserData: function (){
            return axios({
                methods: 'get',
                url: '/apis/GetUserFullDataFromSession',
            });
        }
    },
    mounted:function (){
        ...
        this.loading=true;
        axios.all([
            this.getReply(),this.getUserData()
        ])
            .then(axios.spread(function (res1,res2){
                vueMessage.loading=false;
                $("#message_wrapper").attr("class","message_wrapper")
                console.log("replys",res1);

                console.log("userdata",res2)
                if(res2.data.code==-1){
                    alert("未登陆，将跳转到登陆页面")
